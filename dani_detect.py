...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/Users/dani/bon/bb/python-docs-samples/vision/cloud-client/detect/bonai-240200-9c47d8f8aa13.json"
...
def localize_objects(path):
    ...
    fnt = ImageFont.truetype('/System/Library/Fonts/Courier.dfont', 16)
    cords = {}
    point = 0
    ...
    for object_ in objects:
        ...
        for vertex in object_.bounding_poly.normalized_vertices:
            ...
            vectx = str(point) + 'x'
            vecty = str(point) + 'y'
            cords[vectx] = vertex.x*width
            cords[vecty] = vertex.y*height
            point = point + 1
        draw.polygon([
            cords['0x'], cords['0y'],
            cords['1x'], cords['1y'],
            cords['2x'], cords['2y'],
            cords['3x'], cords['3y']], None, 'red')
        text = '\n{} cf:{}'.format(object_.name, int(object_.score*100))
        draw.text((cords['0x'],cords['0y']), text, font=fnt, fill="red")
    pathSplit = str(path).split('.')
    newfile = pathSplit[0] + '_bb.jpg' 
